...
Six phases, ordered by feasibility, each independently valuable. Phase 1 is a complete product on
...
Two realities bind all six phases and every stretch item; full statements in
...
- **Emergency calls never originate from the desktop** (ADR-0008). Enforcement has two ends: the
  ...
  surfaced read-only. No later phase relaxes this — a remote desktop (Phase 5) is even less
...
- **No software capture of carrier call audio, ever** (ADR-0002). On stock non-rooted Android,
  ...
  No phase assumes otherwise: audio moves only over Bluetooth HFP (Phases 2–4) or a hypothetical
  sanctioned platform API (Phase 6).
...
timeline
    ...
    Phase 3 : Tier B — Linux software HFP via BlueZ and PipeWire
    Phase 4 : Tier B — Windows and macOS HFP via dedicated USB controller
    Phase 5 : Optional cloud account and remote mode — not scheduled
    Phase 6 : Tier C advocacy — sanctioned call-audio companion API
```
...
flowchart LR
    ...
    P3 --> P4["Phase 4 — Tier B USB dongle"]
    P1 -.-> P5["Phase 5 — cloud + remote, unscheduled"]
    P3 -.-> P6["Phase 6 — Tier C advocacy"]
```
...
- Phase 2 needs only Phase 1 plus route targeting on top of the Android `bluetooth/` package that
  ...
  permanent fallback wherever Phase 3/4 prerequisites (BlueZ access, a vetted dongle) are absent.
- Phase 3 builds the OS-independent HFP core once (`tandem_bluetooth::hfp`); Phase 4 reuses it
  unchanged and adds the host stack beneath RFCOMM. The dongle path therefore depends on the HFP
  core being hardened against real AGs in Phase 3 — which is why Linux ships first despite
  Windows/macOS having more users.
- Phases 5 and 6 have no code dependency on 3/4, but Phase 6's advocacy case is strongest with a
...
PipeWire-managed devices through cpal. No special hardware.
...
## Phase 4 — `[Tier B — Win/macOS USB dongle]` HFP via dedicated controller

Same Hands-Free role on Windows and macOS by driving a dedicated USB Bluetooth controller directly,
because those OS stacks do not expose the HF role to applications
...
Done when: flow-e behavior matches Phase 3 on Windows and macOS with the vetted dongle; the probe's
supported verdict is a precondition of every support statement. Users without a dongle stay on
`[Tier B-lite fallback]` permanently and lose nothing from Phase 2.

## Phase 5 — optional cloud account + beyond-same-room mode (explicitly out of scope today)
...
## Phase 6 — `[Tier C — needs vendor support]` sanctioned call-audio companion API advocacy
